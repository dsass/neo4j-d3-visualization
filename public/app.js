...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const graphType = urlParams.get('graph') || 'application'; // Default to 'application'

    // Update page titles to reflect the current graph
    document.title = `${graphType.charAt(0).toUpperCase() + graphType.slice(1)} Graph`;
    document.getElementById('graph-title').textContent = `${graphType.charAt(0).toUpperCase() + graphType.slice(1)} Knowledge Graph`;

    // Fetch data from the backend API
    const response = await fetch(`http://localhost:3000/api/graph/${graphType}`);

    if (!response.ok) {
        console.error("Failed to fetch graph data:", response.status, response.statusText);
        container.innerHTML = `<p style="color: red; text-align: center;">Error: Could not load graph data from the server. Please ensure the server is running and the database is connected.</p>`;
                    ...
                    data.properties.containedNodes.forEach(node => {
                        const listItem = list.append('li')
                            ...
                            .on('mouseout', () => {
                                ...
                                if (!isCollapsed) {
                                    // Remove the highlight from the specific node
    ...
    function clearInfoPanel() {
        ...
        infoPanel.append('div').attr('class', 'property').html(`<span class="property-key">Node Types:</span> <span class="property-value">${groupCount}</span>`);
    ...
    function renderGraph(currentGraph, nodeIdToFocus = null) {
        ...
        nodeGroup.filter(d => d.label === 'Enterprise').append('image')
            ...
            .select(function() { return this.parentNode; }); // pointer-events: none was preventing tooltip
    ...
    simulation = d3.forceSimulation()
        ...
        .force("collide", d3.forceCollide().radius(Math.max(nodeWidth, nodeHeight) / 2 + 5).strength(0.9))
        .on("tick", ticked);
    ...
    function transformGraph() {
        ...
        originalGraph.nodes.forEach(node => {
            ...
            if (!isCollapsed) {
                ...
                // Mark the original node so we know it can be re-collapsed
                node.belongsToSummary = nodeToFinalNodeMap.get(node.id);
            }
        });
